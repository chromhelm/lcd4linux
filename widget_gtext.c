 ...
 * exported functions:
 *
 * WIDGET_CLASS Widget_GText
 *   a simple text widget for GLCD display
 *
 */
...
void widget_gtext_scroll(void *Self)
{
    ...
    int string_len, prefix_len, postfix_len, string_width, pad;
    ...
    string_len = strlen(string) * XRES;
    prefix_len = strlen(prefix) * XRES;
    postfix_len = strlen(postfix) * XRES;

    string_width = T->width - prefix_len - postfix_len;
    if (string_width < 0)
        string_width = 0;
    ...
    switch (T->align) {
        ...
        break;
        case ALIGN_CENTER:
            pad = (string_width - string_len) / 2;
            if (pad < 0)
                pad = 0;
        break;
        case ALIGN_RIGHT:
            pad = string_width - string_len;
            if (pad < 0)
                pad = 0;
        break;
        case ALIGN_AUTOMATIC:
            if (string_len <= string_width) {
                pad = 0;
                break;
            }
        case ALIGN_MARQUEE:
            pad = T->scroll;
            T->scroll++;
            if (T->scroll >=  string_width + postfix_len)
                T->scroll = -string_len - postfix_len;
        break;
    ...
    #define PINGPONGWAIT 2
        ...
        T->value_x_off = prefix_len;
        T->value_x_skip = pad;
        T->postfix_x_off = prefix_len + string_width;

        if(T->postfix_x_off >= T->width)
        {
            T->postfix_x_off = T->width;
        }
        else if(T->postfix_x_off < T->width - postfix_len)
        {
            T->postfix_x_off = T->width - postfix_len;
        }
...
void widget_gtext_update(void *Self)
{
    WIDGET *W = (WIDGET *) Self;
    WIDGET_GTEXT *T = W->data;
    char *string;
...
int widget_gtext_init(WIDGET * Self)
{
    char *section;
    char *c;
    WIDGET_GTEXT *Text;
    ...
    Text = malloc(sizeof(WIDGET_GTEXT));
    memset(Text, 0, sizeof(WIDGET_GTEXT));
    ...
    if (*event_name != '\0') {
    named_event_add(event_name, widget_gtext_update, Self);
	if (Text->update == 1000) {
	    Text->update = 0;
	}
    }
    ...
    timer_add_widget(widget_gtext_update, Self, Text->update, Text->update == 0);

    /* a marquee scroller has its own timer and callback */
    if (Text->align == ALIGN_MARQUEE || Text->align == ALIGN_AUTOMATIC || Text->align == ALIGN_PINGPONG_LEFT
	|| Text->align == ALIGN_PINGPONG_CENTER || Text->align == ALIGN_PINGPONG_RIGHT) {
    timer_add(widget_gtext_scroll, Self, Text->speed, 0);
    }
...
int widget_gtext_quit(WIDGET * Self)
{
    WIDGET_GTEXT *Text;
    if (Self) {
	Text = Self->data;
...
WIDGET_CLASS Widget_GText = {
    .name = "gtext",
    .type = WIDGET_TYPE_XY,
    .init = widget_gtext_init,
    .draw = NULL,
    .quit = widget_gtext_quit,
};
